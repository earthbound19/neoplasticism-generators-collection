...
// for(n = 0; n < rule.length(); n = n + 1)    // javascript
for(int n = 0; n < rule.length(); n = n + 1)
  {
    ...
    // String c = rule.charAt(n);    // javascript
    String c = rule.substring(n, n+1);
    if (c == "A")
      {
        int s_a = A_gr.length;
...
void keyPressed()
{
// int len = s.length;    // javascript
int len = s.length();

if  (key == 'e' || key == 'E' && len > 0)
//    {s = s.substring(0, s.length - 1);}     // javascript
